...
IF EXISTS (SELECT 1
   FROM SYS.SYSREFERENCES R JOIN SYS.SYSOBJECTS O ON (O.ID = R.CONSTID AND O.TYPE = 'F')
   WHERE R.FKEYID = OBJECT_ID('TPP_DOCUMENT') AND O.NAME = 'FK_TPP_DOCU_BANER-DOK_TPP_BANN')
ALTER TABLE TPP_DOCUMENT
   DROP CONSTRAINT "FK_TPP_DOCU_BANER-DOK_TPP_BANN"
go

IF EXISTS (SELECT 1
   FROM SYS.SYSREFERENCES R JOIN SYS.SYSOBJECTS O ON (O.ID = R.CONSTID AND O.TYPE = 'F')
   WHERE R.FKEYID = OBJECT_ID('TPP_DOCUMENT') AND O.NAME = 'FK_TPP_DOCU_BANNER-DO_TPP_BANN')
ALTER TABLE TPP_DOCUMENT
   DROP CONSTRAINT "FK_TPP_DOCU_BANNER-DO_TPP_BANN"
go

IF EXISTS (SELECT 1
   FROM SYS.SYSREFERENCES R JOIN SYS.SYSOBJECTS O ON (O.ID = R.CONSTID AND O.TYPE = 'F')
   WHERE R.FKEYID = OBJECT_ID('TPP_DOCUMENT') AND O.NAME = 'FK_TPP_DOCU_DOCUMENT-_TPP_BANN')
ALTER TABLE TPP_DOCUMENT
   DROP CONSTRAINT "FK_TPP_DOCU_DOCUMENT-_TPP_BANN"
go
...
IF EXISTS (SELECT 1
            FROM  SYSOBJECTS
           WHERE  ID = OBJECT_ID('TPP_BANNER_TYPE')
            AND   TYPE = 'U')
   DROP TABLE TPP_BANNER_TYPE
go
...
IF EXISTS (SELECT 1
           ...
           WHERE  ID    = OBJECT_ID('TPP_USER')
            AND   NAME  = 'USERNAME_UNIQUE'
            AND   INDID > 0
            AND   INDID < 255)
   DROP INDEX TPP_USER.USERNAME_UNIQUE
go
...
/*==============================================================*/
/* Table: TPP_BANNER_TYPE                                       */
/*==============================================================*/
CREATE TABLE TPP_BANNER_TYPE (
   ID                   INT                  NOT NULL,
   NAME                 VARCHAR(255)         NOT NULL,
   PRICE_A              FLOAT                NULL,
   PRICE_B              FLOAT                NULL,
   PRICE_C              FLOAT                NULL,
   CONSTRAINT PK_TPP_BANNER_TYPE PRIMARY KEY (ID)
)
go
...
CREATE TABLE TPP_DOCUMENT (
   ...
   DOC_CUST_NAME        VARCHAR(MAX)         NULL,
   DOC_CUST_ADR         VARCHAR(MAX)         NULL,
   DOC_CUST_COUNTRY     VARCHAR(25)          NULL,
   DOC_CUST_DOC_ID      VARCHAR(50)          NULL,
   ...
   BANNER_ID_1_TYPE     INT                  NULL,
   BANNER_ID_1_AMOUNT   INT                  NULL,
   BANNER_ID_2_TYPE     INT                  NULL,
...
CREATE TABLE TPP_STATUS (
   ...
   ID_RDI_LINK          VARCHAR(255)         NULL,
   ID_NOTE              VARCHAR(255)         NULL,
...
/*==============================================================*/
CREATE UNIQUE INDEX USERNAME_UNIQUE ON TPP_USER (
USERNAME ASC
)
go
...
CREATE TABLE TPP_USER_COMPANY (
   ...
   CONSTRAINT PK_TPP_USER_COMPANY PRIMARY KEY NONCLUSTERED (ID)
)
go
...
ALTER TABLE TPP_DOCUMENT
   ADD CONSTRAINT "FK_TPP_DOCU_BANER-DOK_TPP_BANN" FOREIGN KEY (BANNER_ID_2_TYPE)
      REFERENCES TPP_BANNER_TYPE (ID)
go

ALTER TABLE TPP_DOCUMENT
   ADD CONSTRAINT "FK_TPP_DOCU_BANNER-DO_TPP_BANN" FOREIGN KEY (BANNER_ID_1_TYPE)
      REFERENCES TPP_BANNER_TYPE (ID)
go

ALTER TABLE TPP_DOCUMENT
   ADD CONSTRAINT "FK_TPP_DOCU_DOCUMENT-_TPP_BANN" FOREIGN KEY (BANNER_ID_3_TYPE)
      REFERENCES TPP_BANNER_TYPE (ID)
go
